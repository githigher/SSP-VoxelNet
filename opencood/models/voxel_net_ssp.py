# -*- coding: utf-8 -*-
"""
@Time ： 2024/3/25 23:40
@Auth ： YongTong Gu
@File ：voxel_net_ssp.py
@IDE ：PyCharm
@Motto：悟已往之不谏,知来者之可追
...
from opencood.models.fuse_modules.TriFocusTransformer import TriFocusFusion
from opencood.models.sub_modules.sparse_spatial_pooling import ssp
...
class SSPVoxelNet(nn.Module):
    def __init__(self, args):
        super(SSPVoxelNet, self).__init__()
        self.svfe = PillarVFE(args['pillar_vfe'],
                              num_point_features=4,
                              voxel_size=args['voxel_size'],
                              point_cloud_range=args['lidar_range'])
        self.cml = CML()
        self.fusion_net = TriFocusFusion(feature_dim=128)
        ...
        if 'compression' in args and args['compression'] > 0:
            ...
            self.compression_layer = NaiveCompressor(128, args['compression'])
    ...
    def forward(self, data_dict):
        ...
        vwfs = self.svfe(batch_dict)['pillar_features']  # torch.Size([9091, 64])
        voxel_coords = torch_tensor_to_numpy(voxel_coords)
        vwfs = self.voxel_indexing(vwfs, voxel_coords)  # torch.Size([2, 64, 10, 208, 704])

        # convolutional middle network
        vwfs = self.cml(vwfs)  # torch.Size([2, 64, 2, 208, 704])
        ...
        # compression layer
        if self.compression:
            ego_feature = vmfs[0:1]
            other_feature = vmfs[1:]
            other_compress_feature = self.compression_layer(other_feature)
            vmfs = torch.cat([ego_feature, other_compress_feature], 0)

        # Sparse Spatial Pooling
        vmfs = self.ssp(vmfs)

        # TriFocus Transformer
        vmfs_fusion = self.fusion_net(vmfs, record_len)  # torch.Size([1, 128, 208, 704])

        # region proposal network
        # merge the depth and feature dim into one, output probability score
        # map and regression map
        psm, rm = self.rpn(vmfs_fusion)  # torch.Size([1, 2, 104, 352])
